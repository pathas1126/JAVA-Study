	...
	public DB_Person() {
		...
		try {
			...
			try {
				...
				rs = st1.executeQuery("SELECT * FROM PERSON");
			} catch (Exception createError) {
				System.out.println(createError);
				int resSt1 = st1
						.executeUpdate("CREATE TABLE PERSON(NAME VARCHAR2(15), AGE VARCHAR2(3), TEL VARCHAR2(20))");
				System.out.println("테이블 생성: " + resSt1);
			}

			// 실행시 기존 테이블에 저장된 데이터 출력
			rs = st1.executeQuery("SELECT * FROM PERSON");
			Object[] display = new Object[3];

			while (rs.next()) {
				display[0] = rs.getString("NAME");
				display[1] = rs.getString("AGE");
				display[2] = rs.getString("TEL");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == tf1) { // 이름
			...
			try {
				...
				rsq = st2.executeQuery("SELECT * FROM PERSON");

				while (rsq.next()) {
					data[0] = rsq.getString("NAME");
					data[1] = rsq.getString("AGE");
					data[2] = rsq.getString("TEL");
				}
				// Model에 데이터 입력
