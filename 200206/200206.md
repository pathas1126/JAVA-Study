...
| OutputStreamWriter | 파일에 바이트 단위로 출력한 자료를 문자로 변화해 주는 보조 스트림 |
| BufferdWriter      | 문자로 쓸 때 배열을 제공하여 한꺼번에 쓸 수 있는 기능을 제종해 주는 보조 스트림 |
...
> 입출력 대상이 되는 파일이나 네트워크에 직접 읽거나 쓰는 기능은 없음
