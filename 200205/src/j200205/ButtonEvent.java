...
public class ButtonEvent extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// 이벤트 발생 컴포넌트 정보 얻어오기
		// ActionEvent형 객체 e를 Object형 객체로 형변환해서 가져옴
		Object o = e.getSource();
		// o 객체가 JButton의 인스턴스인지 확인
		if (o instanceof JButton) {
			JButton b = (JButton) o;
			// 시작 버튼을 눌렀다면
			if (s.contentEquals("시작")) {
				// getContentPane()은 JFrame에 색을 부여할 때만 사용
				b.setBackground(new Color(200, 20, 40));
				System.out.println(b1.getBackground());
				this.setTitle(s);
			} else if (s.contentEquals("Go")) {
				b.setBackground(new Color(100, 80, 60));
				this.setTitle(s);
			} else if (s.contentEquals("Back")) {
				b.setBackground(new Color(50, 60, 200));
				this.setTitle(s);
			} else if (s.contentEquals("점프")) {
				b.setBackground(new Color(50, 200, 60));
				this.setTitle(s);
			}
		} else { // 이벤트 발생 객체가 JTextFields
			JTextField tf2 = (JTextField) o;
			tf2.setText("객체형변환이 아~~~주 중요해요!!");
