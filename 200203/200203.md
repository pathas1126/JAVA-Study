...
|           *indexOf(int ch)*           | String 클래스 일반 메소드, 객체명.indexOf(int ch)로 사용<br />해당 문자가 문자열에 위치한 인덱스를 반환,<br />해당 문자가 없다면 '-1'을 리턴 |
...
|              메소드              |                             설명                             |
| :------------------------------: | :----------------------------------------------------------: |
| *Character.isUpperCase(char ch)* | Character 클래스 정적 메소드, Character.isUpperCase()로 사용<br />인자로 전달된 문자가 대문자인 경우 true반환<br />소문자인 경우 false 반환 |
...
public class ConvertText {
	...
	public static void main(String[] args) {
		...
		// 입력받은 문자열 각각의 문자가 담길 변수에 임의의 값으로 초기화
		char ch = 'a';
